...
vec4 applySprings(in vec4 samplerData, in vec4 n)
{
    ...
    storage.r = a * nsum + ((2.0 - udamp) - 4.0 * a) * samplerData.r - (1.0 - vdamp) * samplerData.g;

    // Calculate normal and store in blue-alpha channel
    storage.ba = 2.0 * (n.xz - n.yw);
